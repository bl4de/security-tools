...
def build_jwt(header, payload, key, alg='hmac'):
    message = encode_part(header) + '.' + encode_part(payload)
    if alg == 'hmac':
        signature = hmac.new(bytes(key,'ascii'), bytes(message, 'ascii'), hashlib.sha256).hexdigest()
    elif alg == 'none':
...
jwt = build_jwt(header, payload, 'key')
print(jwt)
