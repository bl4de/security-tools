...
def file_type(file_signature):
    ...
    for signature in FILE_SIGNATURES:
        if file_signature.decode('utf-8').startswith(signature.upper()):
            recognized = FILE_SIGNATURES[signature]

    return "{}\n[+] File type: {}{}{}".format(
        COLORS["cyan"], COLORS["yellow"], recognized, COLORS["white"]
    )
...
def format_chunk(chunk, start, stop, df_chunk=False, dec=False):
    ...
    if dec:
        if df_chunk:
            return " ".join(
                "{}:{}{:#04}{} ".format(
                    make_color(c, df_c), COLORS["grey"], ord(
                        c), COLORS["white"]
                )
        ...
        return " ".join(
            "{}:{}{:#04}{} ".format(
                make_color(c), COLORS["grey"], ord(c), COLORS["white"]
            )
            for c in chunk[start:stop]
        )
    else:
        if df_chunk:
            return " ".join(
...
def extract_shellcode(start, end, read_binary):
    ...
    print(
        "\n{}[+] Shellcode extracted from byte(s) {:#08x} to {:#08x}:{}".format(
            COLORS["cyan"], start, end, COLORS["white"]
        )
    )
    print("\n{}\n".format(shellcode))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--start", help="Start byte")
    parser.add_argument("-e", "--end", help="End byte")
    parser.add_argument("-D", "--diff", help="Perform diff with FILENAME")
    parser.add_argument(
        "-S",
        "--shellcode",
        help="Extract shellcode (-s and -e has to be passed)",
        action="store_true",
    )
    parser.add_argument(
        "-u",
        "--uppercase",
        help="Display output using uppercase characters",
        action="store_true",
    )
    ...
    if arguments.diff:
        diff_file = open(arguments.diff, "rb")
    ...
    if arguments.file:
        ...
        with open(arguments.file, "rb") as infile:
            if (
                ...
                __FROM = int(arguments.start, 16)
                __TO = int(arguments.end, 16)
            else:
                __TO = os.path.getsize(arguments.file)
            ...
            while offset < __TO:
                ...
                if arguments.diff is not None:
                    ...
                    df_output += (
                        format_chunk(df_chunk, 4, 8, chunk,
                                     arguments.decimal) + "   "
                    )
                    df_output += (
                        format_chunk(df_chunk, 8, 12, chunk,
                                     arguments.decimal) + "   "
                    )
                    df_output += (
                        format_chunk(df_chunk, 12, 16, chunk,
                                     arguments.decimal) + df_text
                    )
